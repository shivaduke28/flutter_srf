...
void main() {
  ...
  group('AudioPlayerService Error Handling', () {
    ...
    test('playFile() throws FileNotFoundException for non-existent file', () async {
      ...
      // Act & Assert
      expect(() async => await service.playFile(nonExistentFile), throwsA(isA<FileNotFoundException>()));
    });
    ...
    test('seek() throws ValidationException for negative duration', () async {
      ...
      // Act & Assert
      expect(() async => await service.seek(negativeDuration), throwsA(isA<ValidationException>()));
    });

    test('setVolume() throws ValidationException for out-of-range volume', () async {
      // Act & Assert - Too low
      expect(() async => await service.setVolume(-0.1), throwsA(isA<ValidationException>()));

      // Act & Assert - Too high
      expect(() async => await service.setVolume(1.1), throwsA(isA<ValidationException>()));
    });
